...
public class LeaveAllocationRepository : GenericRepository<LeaveAllocation>, ILeaveAllocationRepository
{
  ...
  public async Task<LeaveAllocation> GetLeaveAllocationWithDetails(int id)
  {
    var leaveAllocation = await _context.LeaveAllocations
      .Include(q => q.LeaveType)
      .FirstOrDefaultAsync(q => q.Id == id) ?? throw new KeyNotFoundException($"Leave allocation with id '{id}' was not found.");
  ...
  public async Task<LeaveAllocation> GetUserAllocations(string userId, int leaveTypeId)
  {
    ...
    return leaveAllocation ?? throw new KeyNotFoundException($"Leave allocation for user '{userId}' and leave type '{leaveTypeId}' was not found.");
  }
}
